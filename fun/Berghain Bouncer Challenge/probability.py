...
def play_probability_aware(cfg: GameConfig) -> int:
    ...
    while state.get("status") == "running":
        ...
        # Decide using probability-aware reservation
        ...
        if constraints_met:
            ...
            # Contributor: always accept and apply decrement(s)
            decision = True
            tstate.apply(decs)

        else:
            # Non-contributor: accept iff expected future contributors still cover needs
            needed = tstate.needed_list()
            p_contrib = attr_stats.p_union_needed(needed)
            ...
            if cfg.verbose:
                print(
                    f"[reserve] needed={needed} p_contrib≈{p_contrib:.4f} "
                    f"seats_after_if_accept={seats_after_if_accept} "
                    f"expected_hits≈{expected_hits:.2f} vs R={R}"
                )
